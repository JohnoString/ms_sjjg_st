...
private:
	...
	void recursive_remove(Node* pNode, const T& key)
	{
		...
		else//关键字key不在节点pNode中
		{
			...
			{
				Node* pLeft = i > 0 ? pNode->pChild[i - 1] : nullptr;  //左兄弟节点
				Node* pRight = i < pNode->keyNum ? pNode->pChild[i + 1] : nullptr;//右兄弟节点
				int j;
